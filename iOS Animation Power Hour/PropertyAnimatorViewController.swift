...
extension PropertyAnimatorViewController {
    func createBasicsSection() -> DemoSectionView {
        ...
        var startAction, pauseAction, resetAction: DemoSectionViewButtonAction?
        ...
        startAction = DemoSectionViewButtonAction(buttonTitle: "Start/Resume", actionBlock: {
            startAction?.button?.isEnabled = false
        ...
        pauseAction = DemoSectionViewButtonAction(buttonTitle: "Pause", startsEnabled: false, actionBlock: {
    ...
    func createStatesSection() -> DemoSectionView {
        ...
        var startAction, pauseAction, stopAction, finishAction, resetAction: DemoSectionViewButtonAction?
        ...
        stopAction = DemoSectionViewButtonAction(buttonTitle: "Stop", startsEnabled: false, actionBlock: {
            startAction?.button?.isEnabled = false
            pauseAction?.button?.isEnabled = false
            stopAction?.button?.isEnabled = false
        ...
        finishAction = DemoSectionViewButtonAction(buttonTitle: "Finish", startsEnabled: false, actionBlock: {
    ...
    func createReverseSection() -> DemoSectionView {
        ...
        var startAction, changeDirectionAction, resetAction: DemoSectionViewButtonAction?
        ...
        changeDirectionAction = DemoSectionViewButtonAction(buttonTitle: "Change direction", actionBlock: {
    ...
    func createScrubbingSection() -> DemoSectionView {
        ...
        let sliderAction = DemoSectionViewSliderAction { sliderValue in
            propertyAnimator.fractionComplete = CGFloat(sliderValue)
        }
        
        let animationSection = DemoSectionView(title: sectionTitle, description: description, sampleCode: sampleCode, animatingView: animatingView, sliderAction: sliderAction)
        return animationSection
    }
    
    func createAddAnimationSection() -> DemoSectionView {
        ...
        var startAction, addAlphaAction, addScaleAction, resetAction: DemoSectionViewButtonAction?
        ...
        startAction = DemoSectionViewButtonAction(buttonTitle: "Start", actionBlock: {
            startAction?.button?.isEnabled = false
        ...
        addAlphaAction = DemoSectionViewButtonAction(buttonTitle: "Add Alpha Animation", startsEnabled: false, actionBlock: {
            startAction?.button?.isEnabled = false
        ...
        addScaleAction = DemoSectionViewButtonAction(buttonTitle: "Add Color Animation", startsEnabled: false, actionBlock: {
            startAction?.button?.isEnabled = false
    ...
    func createTimingCurvesSection() -> DemoSectionView {
        ...
        var firstBezierAction, secondBezierAction, resetAction: DemoSectionViewButtonAction?
        
        var propertyAnimator: UIViewPropertyAnimator?
        
        firstBezierAction = DemoSectionViewButtonAction(buttonTitle: "Animate Bézier 1", actionBlock: {
        ...
        secondBezierAction = DemoSectionViewButtonAction(buttonTitle: "Animate Bézier 2", actionBlock: {
            firstBezierAction?.button?.isEnabled = false
            secondBezierAction?.button?.isEnabled = false
            resetAction?.button?.isEnabled = true
    ...
    func createTimingObjectsSection() -> DemoSectionView {
        let sectionTitle = "Timing Curve Providers"
        let description = "You can store animation parameters like curve, control points, or damping in a timing curve provider, along with a handful of other, more advanced animation parameters. A timing provider is any object conforming to `UITimingCurveProvider`. This timing object is basically a way to wrap all the settings for an animation into a convenient little package, especially useful if there are multiple places in your app that use the same animation.\n\nA timing object has three properties: their type and timing parameters for either cubic (Bézier-based) or spring-based animations. Which of the two timing parameters the object uses depends on the type. For example, if you set the `type` to `builtin` or `cubic`, you only have to provide the cubic timing parameters. This cubic parameters object can be created with one of the default `UIViewAnimationCurve`s, (as described in 'Basics') or a Bézier curve made with two control points. If the type is `spring`, you are expected to provide a spring timing parameter object, which is created with a combination of damping, initial velocity, and/or mass and stiffness."
